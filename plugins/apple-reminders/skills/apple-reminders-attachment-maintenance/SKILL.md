...
For consolidation, inspect all dependencies, copy and verify every destination image first, then hand any authorized source deletion to [organize and cleanup](../apple-reminders-organize-cleanup/SKILL.md). After each copy, use the returned fresh destination reference for the next mutation. Stop the chain on a stale, missing, ambiguous, pending, partial, or manual-repair result.
