...
1. Define an exact list, section, query, date window, completion state, supported sort, or candidate limit. If absent, enumerate lists and ask for or infer one bounded list from current context. For UI-relative “top” or “visible” targets, directly observe the current UI and resolve every visible row to one exact ID, or use an explicit bounded API snapshot. Stop on duplicate UI-to-ID mapping and report which evidence basis was used, because API order is not current UI order.
...
3. Use `inspect_reminder_native` with `kind=sections` and exact `list_id` when the proposal depends on section structure. Never treat section names as global.
...
6. Apply Core changes through `change_reminder`: `set_completion`, `move_to_list`, or `patch`. Apply deletion through `delete_reminder` only after dependency work. Create a section with `create_reminder_section {list_id, name}`. Move to a section or add/remove a tag with `organize_reminder {reference, action}`.
...
Use `inspect_recently_deleted {kind:"list"}` only for bounded discovery; it issues no write reference. For each exact target, call item mode immediately before recovery and pass its fresh opaque `del1`, an exact compatible same-account destination `list_id`, and a unique idempotency key to `recover_deleted_reminder`. A `del1` is short-lived, one-use, and cannot substitute for an active `rev1`. Recover one item at a time and halt on the first non-verified receipt.
...
For “top four, delete, recover, and consolidate images,” first establish whether “top” comes from a directly observed current UI or an explicit bounded API snapshot; never conflate the two. If the items are active, inspect and copy every required image to the destination, verify it there, then re-read and delete sources only if deletion remains authorized. If they are already deleted, recover exact items through fresh `del1` references before reading active source attachments. In both paths, source deletion is last and one-at-a-time.
