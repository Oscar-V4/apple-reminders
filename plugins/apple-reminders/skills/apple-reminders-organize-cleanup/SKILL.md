...
Use `inspect_recently_deleted {kind:"list"}` only for bounded discovery; it issues no write reference. Follow a Recently Deleted cursor only with the identical account and limit. On `pagination_snapshot_stale`, discard the partial inventory and restart without a cursor. For each exact target, call item mode immediately before recovery and pass its fresh opaque `del1`, an exact compatible same-account destination `list_id`, and a unique idempotency key to `recover_deleted_reminder`. Compare the deleted item's `account_id` with the destination list's `source.id`; duplicate display names are not account identity. A `del1` is short-lived, one-use, and cannot substitute for an active `rev1`. Recover one item at a time and halt on the first non-verified receipt.
