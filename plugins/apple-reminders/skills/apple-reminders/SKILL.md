...
9. After a write, trust only the returned Receipt. `verified` requires a fresh identifier-based read. When the resulting Reminder has a relative alarm, a due or alarm change must match the dependent `due` and complete `alarms` array; a list move must also match the destination. `committed_verification_pending` and `partial_success` require another fresh read before any write.
