...
7. To recover an exact item retained in Recently Deleted, use `inspect_recently_deleted` list mode only for bounded discovery, then item mode with the exact ID immediately before recovery. Pass its fresh opaque `del1`, an exact same-account destination `list_id`, and a unique idempotency key to `recover_deleted_reminder`. Recover one item at a time and stop on any non-verified result.
