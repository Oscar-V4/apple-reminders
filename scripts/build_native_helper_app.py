#!/usr/bin/env python3
"""Build and sign the universal macOS Native helper app.
...
EXECUTABLE_NAME = "apple-reminders-image-helper"
EXECUTABLES = {"image": EXECUTABLE_NAME, "sections": "apple-reminders-sections-helper", "recovery": "apple-reminders-recovery-helper"}
SOURCES = {"image": Path("scripts/remkit_attach_image.m"), "sections": Path("scripts/remkit_sections.m"), "recovery": Path("scripts/remkit_recover.m")}
...
def build_app(
    ...
    *,
    ...
    sources = {kind: _regular_file(plugin_root / relative, "reviewed Native source") for kind, relative in SOURCES.items()}
    sdk_path = run([XCRUN, "--sdk", sdk, "--show-sdk-path"]).stdout.strip()
    if not Path(sdk_path).is_dir():
        raise BuildFailure("selected macOS SDK does not exist")
    info_bytes = versioned_info_plist(plugin_root)

    output_app.parent.mkdir(parents=True, exist_ok=True)
    staging_root = Path(
        tempfile.mkdtemp(prefix=".native-helper-", dir=output_app.parent)
    )
    try:
        ...
        for kind, executable in EXECUTABLES.items():
            ...
            for architecture in ARCHITECTURES:
                slice_path = slices / f"{kind}-{architecture}"
                frameworks = ["Foundation"]
                if kind in {"image", "sections"}:
                    frameworks.append("AppKit")
                if kind == "image":
                    frameworks.append("ImageIO")
                run([XCRUN, "--sdk", sdk, "clang", "-isysroot", sdk_path,
                     "-fobjc-arc", "-fblocks", f"-mmacosx-version-min={MINIMUM_MACOS}",
                     "-arch", architecture,
                     *[part for framework in frameworks for part in ("-framework", framework)],
                     str(sources[kind]), "-sectcreate", "__TEXT", "__info_plist", str(info_path),
                     "-o", str(slice_path)])
                slice_paths.append(slice_path)
            binary = macos / executable
            run([XCRUN, "lipo", "-create", *map(str, slice_paths), "-output", str(binary)])
            binary.chmod(0o755)
            sign = [CODESIGN, "--force", "--sign", identity, "--identifier", BUNDLE_IDENTIFIER]
            if identity != "-":
                sign += ["--options", "runtime", "--timestamp"]
            run([*sign, str(binary)], timeout=300)
        ...
        for executable in EXECUTABLES.values():
            run([XCRUN, "lipo", str(macos / executable), "-verify_arch", *ARCHITECTURES])
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--sdk", default="macosx", help="explicit xcrun macOS SDK selector or SDK path")
    ...
    try:
        ...
        parser.exit(1, f"Native helper build failed: {exc}\n")
