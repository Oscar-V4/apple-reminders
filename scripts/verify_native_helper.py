#!/usr/bin/env python3
"""Verify a built, signed, and optionally notarized Native helper app.

Verification is structural and never launches any helper or reads Reminders data.
"""
...
from build_native_helper_app import (
    APP_NAME,
    ARCHITECTURES,
...
def current_build_environment() -> dict[str, str]:
    commands = {
        ...
        "linker": [XCRUN, "ld", "-v"],
        "macos_sdk": [XCRUN, "--sdk", os.environ.get("NATIVE_HELPER_SDK", "macosx"), "--show-sdk-version"],
        "macos_sdk_path": [XCRUN, "--sdk", os.environ.get("NATIVE_HELPER_SDK", "macosx"), "--show-sdk-path"],
...
def _verify_no_entitlements(app: Path) -> None:
    ...
    if completed.returncode != 0:
        raise BuildFailure("could not inspect Native helper entitlements")
...
def verify_app(
    ...
    *,
    ...
    """Verify the bundle and all three executables without native execution."""
    ...
    for kind, executable in EXECUTABLES.items():
        ...
        if set(lipo) != set(ARCHITECTURES) or len(lipo) != len(ARCHITECTURES):
            raise BuildFailure(f"{kind} architectures drift: {lipo}")
        minimum_versions[kind] = {arch: _minimum_version(binary, arch) for arch in ARCHITECTURES}
        if set(minimum_versions[kind].values()) != {MINIMUM_MACOS}:
            raise BuildFailure(f"{kind} minimum macOS drift")
    ...
    expected_inventory = {
        ...
        *(f"{APP_NAME}/Contents/MacOS/{name}" for name in EXECUTABLES.values()),
    ...
    for signed_target in [app, *(app / "Contents/MacOS" / name for name in EXECUTABLES.values())]:
        run(
            [
                CODESIGN,
                "--verify",
        ...
        signing = _codesign_details(signed_target)
        ...
        _verify_no_entitlements(signed_target)
    if require_notarized:
    ...
    return {
        ...
        "binary_sha256": {kind: sha256(app / "Contents/MacOS" / name) for kind, name in EXECUTABLES.items()},
...
def verify_manifest(
    ...
    *,
    ...
    expected_environment_keys = {"clang", "linker", "macos_sdk", "macos_sdk_path", "xcode_path"}
    if (
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        parser.exit(1, f"Native helper verification failed: {exc}\n")
