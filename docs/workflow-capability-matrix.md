...
| Recently Deleted list → exact item | **Supported with platform boundary** | `inspect_recently_deleted {kind:"list"}` returns a bounded local inventory without writable references. `kind:"item"` re-reads one exact ID, includes bounded attachment metadata, and issues a short-lived opaque `del1`. | Local macOS/private-framework and 30-day-retention boundary. A list result is not authority to recover an item. |
...
| UI-relative selection such as “top four” | **Supported only with an explicit evidence basis** | Directly observe the current UI order and resolve each visible row to one exact ID, or use one explicit bounded API snapshot with exact list/filter/status, supported sort (`due`, `modified`, or `title`), limit, returned IDs, and truncation state. State which basis was used. | UI order and API order are different evidence. Stop when duplicate display values prevent exact UI-to-ID resolution. Snapshot-safe pagination still does not replace exact item revalidation before a write. |
| Cross-reminder image copy | **Supported** | `change_reminder_attachment` action `copy_image` takes a fresh destination `reference`, fresh `source_reference`, exact active source image `attachment_id`, and idempotency key. Both references are consumed as one-use preconditions after dispatch even though the source is unchanged. Success requires exact destination read-back. | This is a guarded native copy, not image export. Both Reminders must be active and exact; source bytes must decode as bounded PNG/JPEG; recover a deleted source first and re-read both sides before each copy. |
| “Consolidate, then delete sources” | **Unsafe composition unless dependency-first** | Inspect all exact sources and the destination, copy each image, verify destination state, then obtain new source references and delete sources one at a time. | Recovery is an exact post-deletion operation, not a planning substitute for preserving dependencies. Stop before deletion when any transfer is unresolved. |
...
| Pagination | **Supported with snapshot drift detection** | An opaque v3 cursor binds identical filters/sort/limit plus a private SHA-256 fingerprint of the ordered Reminder IDs and revisions. Every next-page fetch recomputes it. | Membership or revision drift fails without mutation as `concurrent_modification` / `pagination_snapshot_stale`; restart from page one. The fingerprint is not a write precondition, so each mutation still requires a just-in-time exact read. |
...
1. Establish the selection basis. For literal UI-relative “top,” observe the current Reminders UI, resolve every visible row to one exact ID, and record the observation time. Stop on duplicate or ambiguous row-to-ID mapping. Otherwise capture one bounded API result with an exact list/filter, supported sort, `limit=4`, returned IDs, and truncation state. Never label one basis as the other.
2. If the sources are already deleted, inspect the bounded Recently Deleted list only for discovery. Inspect every chosen item again by exact ID immediately before recovery, then pass its fresh `del1` to `recover_deleted_reminder` with an exact same-account destination list and a unique idempotency key. Recover one at a time and stop on the first non-verified receipt.
...
5. If deletion remains part of the authorized goal, re-read every source after all copies are verified, then delete exact sources one at a time. Stop the whole chain on the first uncertain outcome.
